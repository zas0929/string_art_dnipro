...
test("local route optimization is deterministic and never increases image error", () => {
  const configuration = {
    pointCount: 72,
    size: 56,
    subpixelCoverage: getOpticalThreadCoverage(0.19),
  ...
  assert.equal(first.sequence.length, 421);
  assert.ok(
    first.optimization.accepted > 0,
    `expected at least one useful replacement: ${JSON.stringify(first.optimization)}`,
  );
  assert.ok(
    first.residualError < first.residualErrorBeforeOptimization,
    `expected lower residual error: ${JSON.stringify(first)}`,
  );
