...
export class OpticalRoutePlanner {
  ...
  optimizeWeakVertices({
    ...
    const exactScaleWeights = this.scales.length === 1
      ? [1]
      : getScaleWeights(1);
    let attempted = 0;
    let accepted = 0;
    ...
    for (const weakWindow of weakWindows) {
      ...
      for (let candidate = 0; candidate < this.pointCount; candidate++) {
        if (
          candidate === currentMiddle
          || !this.isReplacementAllowed(index, candidate)
        ) {
          continue;
        }
      ...
      if (bestCandidate !== currentMiddle && bestGain > minimumGain) {
        ...
        this.nailUsage[currentMiddle] = Math.max(0, this.nailUsage[currentMiddle] - 1);
        this.nailUsage[bestCandidate]++;
        accepted++;
        totalGain += bestGain;
  ...
  rankWeakWindows() {
    ...
    for (let index = 1; index < this.sequence.length - 1; index++) {
      const from = this.sequence[index - 1];
      const middle = this.sequence[index];
      const to = this.sequence[index + 1];
      const removalGain = this.scoreVertexReplacement(
        from,
        middle,
        to,
        null,
      ...
      windows.push({ index, removalGain });
    }
...
function accumulateKernelDelta(deltaByIndex, kernel, multiplier) {
  ...
  if (kernel.fractions) {
    for (let i = 0; i < kernel.indices.length; i++) {
      ...
      add(
        kernel.indices[i],
        multiplier * kernel.coverage * (1 - fraction),
      );
      if (fraction > 0) {
        add(
