...
// Frozen planner from commit 3423ba4 ("Improve create mode steps").
// Keep v5 reproducible while v6 evolves independently.
export class LegacyOpticalRoutePlanner {
  constructor({
    points,
    lineCount,
    minSkip,
  ...
  adjustRouteScore(score, from, candidate, recentDistanceMean, progressRatio) {
    ...
    const averageDirectionUsage = (progressRatio * this.lineCount + 1)
      / this.directionUsage.length;
  ...
  getScaleLineSamples(from, to, scale) {
    ...
    for (let i = 0; i < fullSamples.length; i++) {
      ...
      const scaledIndex = Math.floor(y / scale.factor) * scale.size
        + Math.floor(x / scale.factor);
